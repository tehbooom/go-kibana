...
func TestAPISpacesGetAll(t *testing.T) {
	mockStatus := SpacesGetAllResponseBody{}

	testCases := []struct {
		name         string
		request      *SpacesGetAllRequest
		statusCode   int
		...
		},
		{
			...
			request: &SpacesGetAllRequest{Params: SpacesGetAllRequestParams{
				IncludeAuthorizedPurposes: BoolPtr(true),
			}},
			responseBody: mockStatus,
			statusCode:   200,
			responseErr:  nil,
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			spacesFunc := api.newSpacesGetAll()
			...
			AssertRequestPath(t, req, "/api/spaces/space")
